					...
					.DoIf(method => method != null, method =>
					{
						...
						if (IsModMethod(method) && seenAssemblies.Add(assembly))
						{
							...
							if (metaData != null && metaData.IsCoreMod == false)
							{
								if (n == 0)
								{
									var modName = GetModMetaData(typeof(ExceptionHandler).Assembly).Name;
									_ = result.Append($"{modName} has installed an exception analyzer. See log for full information.\n" +
