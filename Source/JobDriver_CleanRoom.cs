...
public class JobDriver_CleanRoom : JobDriver_Thoroughly
{
	...
	public override IEnumerable<LocalTargetInfo> CanStart(Pawn thePawn, LocalTargetInfo clickCell)
	{
		...
		if (thePawn.workSettings == null || thePawn.WorkTypeIsDisabled(cleanDef)) return null;
		if (Achtung.Settings.ignoreAssignments == false && thePawn.workSettings.GetPriority(cleanDef) == 0) return null;

		if (AllFilth(clickCell.Cell).Any()) return [clickCell];
		return null;
	}
