...
public class MultiActions
{
	...
	public IEnumerable<string> GetKeys() => allActions.Select(action => action.Key).Distinct();
	...
	public static IEnumerable<Colonist> ColonistsForActions(IEnumerable<MultiAction> subActions)
		=> subActions.Select(action => action.colonist).Distinct();
	...
	public List<FloatMenuOption> GetAutoTakeableActions()
	{
		...
		static bool condition(MultiAction action) => action.option.Disabled == false && action.option.autoTakeable == false;
		if (keys.Any(key => ActionsForKey(key).Any(condition))) return options;
